...
一些可用的 E 系列阵列型号如下所示。有关 E 系列阵列型号及其硬件规格的完整列表，请参见 NetApp Hardware Universe，网址为 https://hwu.netapp.com["NetApp Hardware Universe"^]。

* EF50 系列 -- 入门级全闪存，全 NVMe
* EF80 系列 -- 中档全闪存，全 NVMe
* E4000 系列—入门级混合
...
NOTE: 对于 SANtricity 操作系统 11.80GA 或更高版本，在 SANtricity 操作系统运行时，所有 USB 端口都被禁用。
...
* 控制器机架中最多支持 24 个 NVMe SSD 驱动器。
* 每个控制器支持一个或两个主机接口卡。
* 支持用于双控制器通信的控制器间卡和电缆。
+

NOTE: 配置 EF50 系统时，正确连接镜像电缆至关重要。有关详细信息，请参见 https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections["用缆线连接控制器间镜像连接 - EF50 和 EF80"]。
...
* 控制器机架中最多支持 24 个 NVMe SSD 驱动器。
* 每个控制器支持一个、两个或三个主机接口卡。
* 支持用于双控制器通信的控制器间卡和电缆。
+

NOTE: 配置 EF80 系统时，正确连接镜像电缆至关重要。有关详细信息，请参见 https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections["用缆线连接控制器间镜像连接 - EF50 和 EF80"]。
