...
* 两个独立的网络提供高可用性。请确保使用 VLAN 或两个单独的网络将 iSCSI 流量隔离到与其他类型的流量分开的网络段。
* 已启用发送和接收硬件流量控制 * 端到端 * 。
* 已禁用优先级流量控制。
* 如果适用，请启用巨型帧。
...
.关于此任务
此任务介绍如何从 Hardware 页面访问 iSCSI 端口配置。您也可以从菜单访问配置：Settings[System > Configure iSCSI ports]。
...
. 从左侧面板中，选择菜单：Hardware[Controllers and components]。
...
image::../media/50001_01_conf-win.gif[iSCSI端口标识符]
