...
---
= 在E系列中配置子网管理器—Linux (基于InfiniBand的NVMe)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 在配置子网管理器之前、您必须安装InfiniBand diags软件包以通过获取全局唯一ID (GUID) `ibstat -p` 命令：请参见 xref:nvme-ib-determine-host-port-guids-task.adoc[确定主机端口 GUID 并进行建议的设置] 有关如何安装InfiniBand诊断软件包的信息。
