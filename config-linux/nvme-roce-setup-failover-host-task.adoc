...
.关于此任务
SLES 12 使用设备映射器多路径 (DMMP) 为 NVMe over RoCE 实现多路径。RHEL 8、RHEL 9、RHEL 10、SLES 15 和 SLES 16 使用内置的 Native NVMe 故障转移。根据您运行的操作系统，需要对多路径进行一些额外的配置才能使其正常运行。
...
NOTE: 对于 SLES 15、SLES 16、RHEL 9 和 RHEL 10，默认情况下启用原生 NVMe 多路径，无需额外配置。
