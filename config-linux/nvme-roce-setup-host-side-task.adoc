...
.开始之前
您必须运行最新的兼容 RHEL 9、RHEL 10、SLES 15 或 SLES 16 service pack 操作系统。有关最新要求的完整列表，请参阅 https://mysupport.netapp.com/matrix["NetApp 互操作性表工具"^]。
...
NOTE: 如果使用的是 NVIDIA HCA 并希望使用外部 DOCA 驱动程序，请参阅link:https://developer.nvidia.com/doca-downloads["NVIDIA DOCA 文档"]有关如何在此环境中正确安装驱动程序的信息。
...
+
使用以下内容创建示例文件 `/etc/sysconfig/network/ifcfg-eth4`，其中 MTU 设置为无需数据包碎片即可实现的最高值（通常为 9000）。
...
. 重新启动主机。
+
.. 或者，您可以通过运行以下命令加载  `nvme_rdma` 内核，而无需重新启动：
+
[listing]
----
...
+
.下一步是什么？
在主机上通过 RoCE 设置 NVMe 启动器后，link:nvme-roce-configure-storage-connections-task.html["配置 NVMe over RoCE 端口设置"]。
