...
. 在主机上启用并启动 iSCSI 服务：
+
*红帽企业版 Linux 8、9 和 10（RHEL 8、RHEL 9 和 RHEL 10）*
...
----
+
*SUSE Linux Enterprise Server 12、15 和 16（SLES 12、SLES 15 和 SLES 16）*
...
iscsiadm -m session
----
.. 从阵列中检查 iscsi 会话状态。从 SANtricity System Manager，导航到 *设置* > *系统* > *查看/结束 iSCSI 会话*。
