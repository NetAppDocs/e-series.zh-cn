...
CAUTION: *数据丢失的风险。*您必须在无损环境中通过 RoCE 运行 NVMe。在网络上使用全局暂停帧或优先级流量控制 (PFC) 启用端到端流量控制，以消除数据丢失的风险。PFC 要求将 NVMe over RoCE 标记为 DSCP 值为 `106`的优先级 3 流量。
...
.下一步是什么？
在验证 Linux 配置支持和查看限制后，link:nvme-roce-configure-ip-address-dhcp-task.html["使用 DHCP 配置 IP 地址"]。
