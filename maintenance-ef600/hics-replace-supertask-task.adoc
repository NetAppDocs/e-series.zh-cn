...
.关于此任务
更换故障的 HIC 时，您必须使一个控制器离线，使系统处于非冗余状态，更换 HIC，并使控制器恢复在线。

.开始之前
* 请查看 link:hics-overview-supertask-concept.html["EF300 或 EF600 HIC 升级的要求"]。
* 为此程序安排停机维护时段。在成功完成此过程之前，您无法访问存储阵列上的数据。由于两个控制器在通电时必须具有相同的 HIC 配置，因此在安装 HIC 时，必须将一个控制器脱机才能将系统置于非冗余状态。
* 确保您已具备以下条件：
+
** 与控制器兼容的 HIC 。
...
+
.. 选择菜单：Hardware[Controllers and components]。
.. 选择要置于脱机状态的控制器。
